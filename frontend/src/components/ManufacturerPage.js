...
function ManufacturerPage() {
  ...
  const handleCreatePills = async () => {
    try {
      ...
      const response = await axios.post('/api/pills/create', {
        manufacturer,
        SKU,
        quantity,
        productionDate
      ...
      setQrCodes(response.data.secrets.map(secret => `/consumer/${secret}`));
      console.log('QR Codes:', response.data.secrets.map(secret => `/consumer/${secret}`));
    } catch (error) {
